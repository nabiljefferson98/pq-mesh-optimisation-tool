...
def fig01_energy_reduction():
    ...
    labels = [m.replace("plane_", "").replace("_noisy", "").replace("_", "×") for m in meshes]
    ...
    bars_mac = ax.bar(x - width/2,
                      [mac[m]["energy_reduction"] for m in meshes],
                      width, label="Mac M3 (Numba)", color=MAC_COLOUR, alpha=0.85,
                      edgecolor="white", linewidth=0.5)
    bars_win = ax.bar(x + width/2,
                      [win[m]["energy_reduction"] for m in meshes],
                      width, label="Windows RTX (Numba)", color=WIN_COLOUR, alpha=0.85,
                      edgecolor="white", linewidth=0.5)
    ...
    ax.set_title("Figure 1 — Energy Reduction on Synthetic Planar Grids\n"
...
def fig02_scalability():
    ...
    ax.errorbar(faces, mac_times, yerr=mac_stds, fmt="o-",
                color=MAC_COLOUR, label=f"Mac M3  (α={alpha_mac:.2f})",
                capsize=4, linewidth=1.8, markersize=7)
    ax.errorbar(faces, win_times, yerr=win_stds, fmt="s--",
                color=WIN_COLOUR, label=f"Windows RTX  (α={alpha_win:.2f})",
                capsize=4, linewidth=1.8, markersize=7)
    ax.plot(x_smooth, c_mac * x_smooth**alpha_mac,
            color=MAC_COLOUR, linewidth=0.8, linestyle=":", alpha=0.6)
    ax.plot(x_smooth, c_win * x_smooth**alpha_win,
            color=WIN_COLOUR, linewidth=0.8, linestyle=":", alpha=0.6)

    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel("Number of Quad Faces (log scale)")
    ax.set_ylabel("Wall-Clock Time / s (log scale)")
    ax.set_title("Figure 2 — Scalability: Wall-Clock Time vs Mesh Resolution\n"
...
def fig03_backend_comparison():
    ...
    meshes = sorted(data["mac_numpy"].keys(),
                    key=lambda m: data["mac_numpy"][m]["n_faces"])
    ...
    for ax, platform, pname, pcolour in [
        ...
        b1 = ax.bar(x - w/2, np_times, w, yerr=np_stds,
                    label="NumPy", color=NUMPY_COLOUR, alpha=0.85,
                    capsize=3, error_kw={"linewidth": 1.2}, edgecolor="white")
        b2 = ax.bar(x + w/2, nb_times, w, yerr=nb_stds,
                    label="Numba", color=NUMBA_COLOUR, alpha=0.85,
                    capsize=3, error_kw={"linewidth": 1.2}, edgecolor="white")

        # annotate slowdown ratios
        for i, (np_t, nb_t) in enumerate(zip(np_times, nb_times)):
            ratio = nb_t / np_t
            ax.text(x[i], max(np_t, nb_t) * 1.05,
                    f"{ratio:.1f}×", ha="center", fontsize=8, color="#6B7280")
    ...
    fig.suptitle("Figure 3 — Backend Comparison: NumPy vs Numba\n"
...
def fig04_convergence():
    ...
    ax.bar(x - w/2, mac_times, w, yerr=mac_stds,
           label="Mac M3", color=MAC_COLOUR, alpha=0.85,
           capsize=4, edgecolor="white")
    ax.bar(x + w/2, win_times, w, yerr=win_stds,
           label="Windows RTX", color=WIN_COLOUR, alpha=0.85,
           capsize=4, edgecolor="white")
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylabel("Wall-Clock Time / s")
    ...
    ax.bar(x - w/2, mac_er, w,
           label="Mac M3", color=MAC_COLOUR, alpha=0.85, edgecolor="white")
    ax.bar(x + w/2, win_er, w,
           label="Windows RTX", color=WIN_COLOUR, alpha=0.85, edgecolor="white")
    ...
    fig.suptitle("Figure 4 — Convergence Analysis: Time and Energy Reduction\n"
...
def fig05_realworld_timing():
    ...
    labels = [f"{m.replace('_quadrangulated','').replace('_quad','')}\n"
              f"({mac[m]['n_faces']:,} faces)" for m in shared]

    x, w = np.arange(len(shared)), 0.35
    fig, ax = plt.subplots(figsize=(9, 5))

    b1 = ax.bar(x - w/2, [mac[m]["time_mean"] for m in shared], w,
                yerr=[mac[m]["time_std"] for m in shared],
                label="Mac M3", color=MAC_COLOUR, alpha=0.85,
                capsize=4, edgecolor="white")
    b2 = ax.bar(x + w/2, [win[m]["time_mean"] for m in shared], w,
    ...
    ax.set_xticklabels(labels)
    ax.set_ylabel("Wall-Clock Time / s")
    ax.set_title("Figure 5 — Real-World Mesh Optimisation Time\n"
...
def fig06_planarity():
    ...
    ax.bar(x - w/2, before_means, w, yerr=before_stds,
           label="Before optimisation", color="#F59E0B", alpha=0.85,
           capsize=4, edgecolor="white")
    ax.bar(x + w/2, after_means, w, yerr=after_stds,
           label="After optimisation",  color="#10B981", alpha=0.85,
           capsize=4, edgecolor="white")

    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylabel("Mean Planarity Deviation (×10⁻³)")
    ax.set_title("Figure 6 — Planarity Improvement: Before vs After Optimisation\n"
    ...
    ax.annotate("*Oloid starts near-planar;\noptimiser trades planarity\nfor angle balance",
                xy=(1, after_means[1]), xytext=(1.6, after_means[1] * 1.8),
...
def fig07_heatmap():
    ...
    exp03_meshes = ["plane_20x20_noisy", "oloid256_quad",
                    "spot_quadrangulated", "blub_quadrangulated"]
    mesh_labels  = ["Plane\n20×20", "Oloid\n256", "Spot", "Blub"]
    ...
    ax.set_title("Figure 7 — Platform Timing Heatmap: Mac M3 vs Windows RTX\n"
